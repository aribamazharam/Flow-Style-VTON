...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(train_loader):
        ...
        face_mask = torch.FloatTensor((data['label'].numpy() == 1).astype(np.int)) + torch.FloatTensor((data['label'].numpy() == 12).astype(np.int))
        other_clothes_mask = torch.FloatTensor((data['label'].numpy() == 5).astype(np.int)) + torch.FloatTensor((data['label'].numpy() == 6).astype(np.int)) \
                             + torch.FloatTensor((data['label'].numpy() == 8).astype(np.int)) + torch.FloatTensor((data['label'].numpy() == 9).astype(np.int)) \
        ...
        concat_un = torch.cat([preserve_mask, densepose, pose], 1)
        flow_out_un = PB_warp_model(concat_un, clothes_un, pre_clothes_edge_un)
        ...
        warped_prod_edge_un = F.grid_sample(pre_clothes_edge_un, last_flow_un.permute(0, 2, 3, 1),
                                            mode='bilinear', padding_mode='zeros')

        flow_out_sup = PB_warp_model(concat_un, clothes, pre_clothes_edge)
        ...
        arm_mask = torch.FloatTensor((data['label'].numpy() == 11).astype(np.float)) + torch.FloatTensor((data['label'].numpy() == 13).astype(np.float))
        hand_mask = torch.FloatTensor((data['densepose'].numpy() == 3).astype(np.int)) + torch.FloatTensor((data['densepose'].numpy() == 4).astype(np.int))
        dense_preserve_mask = torch.FloatTensor((data['densepose'].numpy() == 15).astype(np.int)) + torch.FloatTensor((data['densepose'].numpy() == 16).astype(np.int)) \
                              + torch.FloatTensor((data['densepose'].numpy() == 17).astype(np.int)) + torch.FloatTensor((data['densepose'].numpy() == 18).astype(np.int)) \
                              + torch.FloatTensor((data['densepose'].numpy() == 19).astype(np.int)) + torch.FloatTensor((data['densepose'].numpy() == 20).astype(np.int)) \
                              + torch.FloatTensor((data['densepose'].numpy() == 21).astype(np.int)) + torch.FloatTensor((data['densepose'].numpy() == 22))
        hand_img = (arm_mask * hand_mask) * real_image
        dense_preserve_mask = dense_preserve_mask * (1 - warped_prod_edge_un)
        preserve_region = face_img + other_clothes_img + hand_img

        gen_inputs_un = torch.cat([preserve_region, warped_cloth_un, warped_prod_edge_un, dense_preserve_mask], 1)
        ...
        flow_out = PF_warp_model(p_tryon_un.detach(), clothes, pre_clothes_edge)
        ...
        l1_loss_batch = torch.abs(warped_cloth_sup.detach() - person_clothes)
        l1_loss_batch = l1_loss_batch.reshape(opt.batchSize, 3 * 256 * 192)
        l1_loss_batch = l1_loss_batch.sum(dim=1) / (3 * 256 * 192)
        l1_loss_batch_pred = torch.abs(warped_cloth.detach() - person_clothes)
        ...
        num_all = len(np.where(weight.numpy() > 0)[0])
        if num_all == 0:
            num_all = 1
        ...
        skin_mask = warped_prod_edge_un.detach() * (1 - person_clothes_edge)
        ...
        m_composite1 = torch.sigmoid(m_composite) * warped_prod_edge
        m_composite = person_clothes_edge * m_composite1
        p_tryon = warped_cloth * m_composite + p_rendered * (1 - m_composite)

        loss_mask_l1 = torch.mean(torch.abs(1 - m_composite))
        loss_l1_skin = criterionL1(p_rendered * skin_mask, skin_mask * real_image)
        loss_vgg_skin = criterionVGG(p_rendered * skin_mask, skin_mask * real_image)
        loss_l1 = criterionL1(p_tryon, real_image)
        loss_vgg = criterionVGG(p_tryon, real_image)
        bg_loss_l1 = criterionL1(p_rendered, real_image)
        bg_loss_vgg = criterionVGG(p_rendered, real_image)
        ...
        if step % 1000 == 0:
            ...
            c = clothes
            d = person_clothes
            e = torch.cat([skin_mask, skin_mask, skin_mask], 1)
            f = warped_cloth
            g = p_rendered
